...
import static io.modelcontextprotocol.spec.McpSchema.GetPromptResult;
...
public class ConferencePrompts {
    ...
    @McpPrompt(
            name = "codemash-sessions-per-day",
            description = "Analyze CodeMash 2026 conference sessions and get a breakdown by day plus total session count")
    public GetPromptResult sessionsPerDayPrompt() {
        String message = "As a conference data analyst, please analyze the CodeMash 2026 conference sessions and tell me:\n\n" +
                ...
                "Use the cyc-sessions-by-date tool to get the session counts by date. This tool returns a map with dates as keys and session counts as values. " +
                "Calculate the total by adding up all the daily counts.";
        return new GetPromptResult(
                "CodeMash 2026 sessions analysis by day",
